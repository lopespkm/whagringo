...
const UserSchema = Yup.object().shape({
    ...
    companyName: Yup.string()
        ...
        .max(50, "Muito tempo!")
        .required("Solicitado"),
    password: Yup.string().min(5, "Muito curto!").max(50, "Muito tempo!"),
    email: Yup.string().email("E-mail inválido").required("Solicitado"),
    phone: Yup.string().required("Solicitado"),
});
    ...
    <div className="geral-signup">
      <div className="container-signup">
        <div className="paper">
          ...
          <div>
            <span className="span">
              👋🏻 Comece o seu <b>Teste GRATUITO</b> 3 dias de WhatsApp CRM em apenas 3 passos!{" "}
              <b>Não se preocupe, não pedimos os dados do seu cartão.</b> 💳
            </span>
          </div>
                    ...
                    >
                      {plans.map((plan, key) => (
                        <MenuItem key={key} value={plan.id}>
                          {plan.name} - Agentes: {plan.users} - Conexões: {plan.connections} - Departamentos: {plan.queues} - USD {plan.amount}
                        </MenuItem>
                      ))}
                    </Field>
                  </Grid>
